...
class CredentialApi {
  ...
  Future<String> createCredential(String userId, String service, String token) async {
    if (service.length == 0 || ["GITHUB", "GOOGLE", "SCALEWAY", "COINMARKET", "DOCKER", "ONEDRIVE", "NOTION"]
        .contains(service) == false) {
      throw Exception("Invalid credential");
    }

    try {
      Response response = await dio.post("/user/$userId/storage/credential?service=$service", data: {
        "token": token,
      });
  ...
  Future<void> deleteCredential(String userId, String service) async {
    ...
    try {
      Response response = await dio.delete("/user/$userId/storage/credential?service=$service");
  ...
  Future<void> putCredential(String userId, String service, String token) async {
    if (service.length == 0 || ["GITHUB", "GOOGLE", "SCALEWAY", "COINMARKET", "DOCKER", "ONEDRIVE", "NOTION"]
        .contains(service) == false) {
      throw Exception("Invalid credential");
    }
    ...
    try {
      Response response = await dio.put("/user/$userId/storage/credential?service=$service", data: {
        "token": token,
      });
  ...
  Future<String> getCredential(String userId, String service) async {
    ...
    try {
      Response response = await dio.get("/user/$userId/storage/credential?service=$service");
